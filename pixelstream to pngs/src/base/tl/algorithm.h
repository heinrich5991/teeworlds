...
template<class R>
void sort_quick(R range, int left, int right)
{
    if (left < right)
    {

        static typename R::type pivot;
        pivot = range.index(right);
        int l = left;
        int r = right;
        do
        {
            while (range.index(l) < pivot) l++;
            while (pivot < range.index(r)) r--;
            if (l <= r)
            {
                swap(range.index(r), range.index(l));
                l++;
                r--;
            }
        } while (l <= r);
        sort_quick(range, left, r);
        sort_quick(range, l, right);
    }
}
...
{
	//sort_bubble(range);
	sort_quick(range, 0, range.size() - 1);
}
