...
void CMenus::RenderServerbrowserServerList(CUIRect View)
{
    ...
    if(m_SelectedIndex > -1)
    {
        for(int i = 0; i < m_NumInputEvents; i++)
        {
            ...
            if(NewIndex > -1 && NewIndex < NumServers)
            {
                ...
                if (g_Config.m_UiServersPage == PAGE_SERVERS_RECENT)
    ...
    for (int i = 0; i < NumServers; i++)
    {
        const CServerInfo *pItem;
        if (g_Config.m_UiServersPage == PAGE_SERVERS_RECENT)
            pItem = ServerBrowser()->Get(i);
        else
            pItem = ServerBrowser()->SortedGet(i);
    ...
    for (int i = 0; i < NumServers; i++)
    {
        ...
        const CServerInfo *pItem;
        if (g_Config.m_UiServersPage == PAGE_SERVERS_RECENT)
            pItem = ServerBrowser()->Get(ItemIndex);
        else
            pItem = ServerBrowser()->SortedGet(ItemIndex);
        CUIRect Row;
        ...
        for(int c = 0; c < NumCols; c++)
        {
            ...
            else if(ID == COL_NAME)
            {
                ...
                if(g_Config.m_BrFilterString[0] && (pItem->m_QuickSearchHit&IServerBrowser::QUICK_SERVERNAME))
                {
                    ...
                    bool aNameHighlight[512] = {false};
                    char aBrFilterStringPart[512];
                    int BrFilterStringLastIndex = 0;
                    str_copy(aBrFilterStringPart, g_Config.m_BrFilterString, sizeof(aBrFilterStringPart));
                    for (int iFilter = 0; iFilter <= str_length(g_Config.m_BrFilterString); iFilter++)
                    {
                        if (g_Config.m_BrFilterString[iFilter] == 32 || g_Config.m_BrFilterString[iFilter] == 0)
                        {
                            ...
                            const char *s = str_find_nocase(pItem->m_aName, aBrFilterStringPart);
                            if (s)
                            {
                                for (int i = 0; i < str_length(aBrFilterStringPart); i++)
                                {
                                    aNameHighlight[i + s - pItem->m_aName] = true;
                                }
                            }
                            BrFilterStringLastIndex = iFilter + 1;
                        }
                    }
                    for (int i = 0; i < str_length(pItem->m_aName); i++)
                    {
                        if (aNameHighlight[i])
                            TextRender()->TextColor(0.4f,0.4f,1.0f,1);
                        else
                            TextRender()->TextColor(1,1,1,1);
                        TextRender()->TextEx(&Cursor, pItem->m_aName+i, 1);
                    }
                    TextRender()->TextColor(1,1,1,1);
                }
                else
    ...
    if(NewSelected != -1)
    {
        ...
        const CServerInfo *pItem;
        if (g_Config.m_UiServersPage == PAGE_SERVERS_RECENT)
            pItem = ServerBrowser()->Get(NewSelected);
        else
            pItem = ServerBrowser()->SortedGet(NewSelected);
        str_copy(g_Config.m_UiServerAddress, pItem->m_aAddress, sizeof(g_Config.m_UiServerAddress));
                ...
                for (int iFilter = 0; iFilter <= str_length(g_Config.m_BrFilterString); iFilter++)
                {
                    if (g_Config.m_BrFilterString[iFilter] == 32 || g_Config.m_BrFilterString[iFilter] == 0)
                    {
                        if (sizeof(aBrFilterStringPart) > iFilter - BrFilterStringLastIndex)
                            ...
                            str_copy(aBrFilterStringPart, g_Config.m_BrFilterString + BrFilterStringLastIndex, sizeof(aBrFilterStringPart));
                        const char *s = str_find_nocase(pName, aBrFilterStringPart);
                        if (s)
                        {
                            for (int i = 0; i < str_length(aBrFilterStringPart); i++)
                            {
                                aNameHighlight[i + s - pName] = true;
                            }
                        }
                        BrFilterStringLastIndex = iFilter + 1;
                    }
                }
                for (int i = 0; i < str_length(pName); i++)
                {
                    if (aNameHighlight[i])
                        TextRender()->TextColor(0.4f,0.4f,1.0f,1);
                    else
                        TextRender()->TextColor(1,1,1,1);
                    TextRender()->TextEx(&Cursor, pName+i, 1);
                }
                TextRender()->TextColor(1,1,1,1);
            }
            else
...
void CMenus::RenderServerbrowserFriends(CUIRect View)
{
    ...
    static float s_ScrollValue = 0;
    static int s_GoToFriendButton = 0;
    ...
    for(int i = 0; i < m_pClient->Friends()->NumFriends(); ++i)
    {
        ...
        if(Item.m_Visible)
        {
            CUIRect RectButton = Item.m_Rect;
            Item.m_Rect.Margin(2.5f, &Item.m_Rect);
            if (pFriend->m_Online)
                RenderTools()->DrawUIRect(&Item.m_Rect, vec4(0.0f, 1.0f, 0.0f, 0.5f), CUI::CORNER_ALL, 4.0f);
            else
            ...
            if (pFriend->m_Online && m_FriendlistSelectedIndex == i)
            {
...
void CMenus::RenderServerbrowser(CUIRect MainView)
{
	...
	*/
    ...
    if(DoButton_MenuTab(&s_InternetButton, Localize("Internet"), g_Config.m_UiServersPage==PAGE_SERVERS_INTERNET, &Button, CUI::CORNER_T))
    {
        g_Config.m_UiServersPage = PAGE_SERVERS_INTERNET;
        ServerBrowser()->Refresh(IServerBrowser::TYPE_INTERNET);
    }
    //ViewRest.VSplitLeft(1.0f, &Button, &ViewRest);
    ViewRest.VSplitLeft(100.0f, &Button, &ViewRest);
    static int s_LanButton=0;
    if(DoButton_MenuTab(&s_LanButton, Localize("Lan"), g_Config.m_UiServersPage==PAGE_SERVERS_LAN, &Button, CUI::CORNER_T))
    {
        g_Config.m_UiServersPage = PAGE_SERVERS_LAN;
        ServerBrowser()->Refresh(IServerBrowser::TYPE_LAN);
    }
    //ViewRest.VSplitLeft(1.0f, &Button, &ViewRest);
    ViewRest.VSplitLeft(100.0f, &Button, &ViewRest);
    static int s_FavoriteButton=0;
    if(DoButton_MenuTab(&s_FavoriteButton, Localize("Favorite"), g_Config.m_UiServersPage==PAGE_SERVERS_FAVORITES, &Button, CUI::CORNER_T))
    {
        g_Config.m_UiServersPage = PAGE_SERVERS_FAVORITES;
        ServerBrowser()->Refresh(IServerBrowser::TYPE_FAVORITES);
    }
    //ViewRest.VSplitLeft(1.0f, &Button, &ViewRest);
    ViewRest.VSplitLeft(100.0f, &Button, &ViewRest);
    static int s_RecentButton=0;
    if(DoButton_MenuTab(&s_RecentButton, Localize("Recent"), g_Config.m_UiServersPage==PAGE_SERVERS_RECENT, &Button, CUI::CORNER_T))
    {
        g_Config.m_UiServersPage = PAGE_SERVERS_RECENT;
	...
	// status box
	{
		...
		char aBuf[64];
		if(str_comp(Client()->LatestLuaVersion(), "0") != 0)
		{
		    if (str_comp(Client()->LatestVersion(), "0") != 0)
                str_format(aBuf, sizeof(aBuf), Localize("Teeworlds %s - L%s is out! Download it at nclient.n-lvl.com!"), Client()->LatestVersion(), Client()->LatestLuaVersion());
            else
                str_format(aBuf, sizeof(aBuf), Localize("Teeworlds %s - L%s is out! Download it at nclient.n-lvl.com!"), GAME_VERSION, Client()->LatestLuaVersion());
			TextRender()->TextColor(1.0f, 0.4f, 0.4f, 1.0f);
		}
		else
			str_format(aBuf, sizeof(aBuf), Localize("Current version: %s - L%s"), GAME_VERSION, GAME_LUA_VERSION);
		...
		if(DoButton_Menu(&s_RefreshButton, Localize("Refresh"), 0, &Button) || m_F5Pressed)
		{
...
{
    pfnCallback(pResult, pCallbackUserData);
    if(pResult->NumArguments() && g_Config.m_UiPage == PAGE_SERVERS_FAVORITES && ((CMenus *)pUserData)->Client()->State() == IClient::STATE_OFFLINE)
