...
{
	return round_to_int(CheckPos.x)/32 < -200 || round_to_int(CheckPos.x)/32 > GameServer()->GetTeamCollision(0)->GetWidth()+200 ||
			round_to_int(CheckPos.y)/32 < -200 || round_to_int(CheckPos.y)/32 > GameServer()->GetTeamCollision(0)->GetHeight()+200 ? true : false;
}
bool CEntity::Active()
{
