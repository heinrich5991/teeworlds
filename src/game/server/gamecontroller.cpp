...
bool IGameController::OnEntity(int Index, int Flags, vec2 Pos, int SwitchGroup, bool InvertSwitch)
{
	...
	case ENTITY_CRAZY_BULLET:
		...
		for(int i = 0; i < MAX_CLIENTS; i++)
			new CProjectile(GameServer()->m_TeamsCore.GetTeamWorld(i), WEAPON_SHOTGUN, -1, Pos, Dir, 0,0, false, 0, SOUND_GUN_FIRE, WEAPON_SHOTGUN, SwitchGroup, InvertSwitch, false);
		break;
	}
...
void IGameController::OnReset(int DDRTeam)
{
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		if(GameServer()->m_apPlayers[i] && GameServer()->m_apPlayers[i]->DDRTeam() == DDRTeam)
		{
			...
			if(m_RoundCount == 0)
			{
				GameServer()->m_apPlayers[i]->m_Score = -1;
