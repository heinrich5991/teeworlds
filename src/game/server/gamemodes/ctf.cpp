...
{
	if(IGameController::OnEntity(Index, 0, Pos, SwitchGroup, InvertSwitch))
		return true;
...
void CGameControllerCTF::Tick()
{
	...
	for(int fi = 0; fi < 2; fi++)
	{
		...
		if(GameServer()->GetCollision(F->GameWorld()->ID())->GetCollisionAt(F->m_Pos.x, F->m_Pos.y)&CCollision::COLFLAG_DEATH || F->GameLayerClipped(F->m_Pos))
		{
		...
		else
		{
			...
			for(int i = 0; i < Num; i++)
			{
				if(!apCloseCCharacters[i]->IsAlive() || apCloseCCharacters[i]->GetPlayer()->GetTeam() == TEAM_SPECTATORS || GameServer()->GetCollision(F->GameWorld()->ID())->IntersectLine(F->m_Pos, apCloseCCharacters[i]->m_Pos, NULL, NULL, CCollision::COLFLAG_SOLID))
					continue;
			...
			if(!F->m_pCarryingCharacter && !F->m_AtStand)
			{
				if(Server()->Tick() > F->m_DropTick + Server()->TickSpeed()*30)
				{
					...
					GameServer()->GetCollision(F->GameWorld()->ID())->MoveBox(&F->m_Pos, &F->m_Vel, 0, vec2(F->ms_PhysSize, F->ms_PhysSize), 0.5f);
