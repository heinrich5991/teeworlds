...
class CProxy_Client05_Server06 : public IProxy
{
	int m_MapDownloadSize;
public:
	CProxy_Client05_Server06();
...
IProxy *CreateProxy_Client05_Server06()
{
	return new CProxy_Client05_Server06();
}

CProxy_Client05_Server06::CProxy_Client05_Server06()
	: IProxy()
{
	m_MapDownloadSize = 0;
}

void CProxy_Client05_Server06::TranslateClientPacket(CNetChunk *pPacket, CProxyCB ClientCB, CProxyCB ServerCB)
{
	CUnpacker Unpacker;
	...
	else
	{
		...
		if(Msg == Protocol5::NETMSGTYPE_CL_STARTINFO
			|| Msg == Protocol5::NETMSGTYPE_CL_CHANGEINFO)
		{
			// abuse that both messages have the same layout
...
void CProxy_Client05_Server06::TranslateServerPacket(CNetChunk *pPacket, CProxyCB ClientCB, CProxyCB ServerCB)
{
	CUnpacker Unpacker;
	Unpacker.Reset(pPacket->m_pData, pPacket->m_DataSize);
	CPacker Packer;
...
int CProxy_Client05_Server06::TranslateServerSnap(CSnapshot *pSnap)
{
